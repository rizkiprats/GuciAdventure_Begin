...
public class Tangga_Naik : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        {
            if (collision.GetComponent<PlayerControllers>().ismoveUp())
            {
                Tangga.GetComponent<BoxCollider2D>().isTrigger = false;
                LantaiTingkat.GetComponent<Collider2D>().isTrigger = true;
            ...
            else
            {
                ...
                LantaiTingkat.GetComponent<Collider2D>().isTrigger = false;
            }
        }
